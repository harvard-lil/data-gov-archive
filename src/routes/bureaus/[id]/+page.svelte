  ...
  $effect(async () => {
    ...
    try {
      ...
      const totalCount = await queryData(
        ...
        WHERE aggregation = 'bureaus' AND identifier = $1
      `,
        [identifier]
      );
      ...
      data.label = datasets[0]?.bureau_name || identifier;
    } catch (error) {
      console.error("Error loading bureau datasets:", error);
    }
  });
</script>
...
{#if data.totalItems > 0}
  ...
  />
{:else}
  <p>Loading datasets for bureau...</p>
{/if}
